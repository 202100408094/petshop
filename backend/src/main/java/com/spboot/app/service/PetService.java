...
import com.spboot.app.mapper.PetMapper;
...
public class PetService {
     ...
     *  根据id 获取一行数据
     */
    public R<Pet> findById(Integer id) {
     ...
     *  根据Wrapper 对象进行数据筛选
     */
    public R<List<Pet>> selectAll(Wrapper<Pet> query) {
    ...
    public R selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Pet> wrapper = mapToWrapper(map);
        // 设置排序
    ...
    /**
     *  根据map 条件筛选people字段等于session.username的数据并分页
     *
     */
    public R selectPagesPeople(Map<String, Object> map) {
        // 获取前端给到列表基础参数
        SelectPage selectPage = new SelectPage(map, 10, "id", "DESC");
        // 将map参数转换为mybatis-plus的QueryWrapper类
        QueryWrapper<Pet> wrapper = mapToWrapper(map);
        // 设置为当前角色
        wrapper.eq("people", SessionFactory.getUsername());
        wrapper.orderBy(true, selectPage.isAsc(), selectPage.getOrderby());
        Page page = new Page(selectPage.getPage(), selectPage.getPagesize());
        return selectPages(wrapper, page);
    }
     ...
     */
    public QueryWrapper<Pet> mapToWrapper(Map<String, Object> map) {
        // 创建 QueryWrapper 对象
        QueryWrapper<Pet> wrapper = Wrappers.query();
    ...
    public R selectPages(QueryWrapper<Pet> wrapper, IPage page) {
     ...
     * @return 是否处理成功
     */
    public R insert(Pet entityData, Map post) {
        // 判断是否有填写宠物编号。
        if (StringUtil.isNullOrEmpty(post.get("numbers"))) {
            return R.error("请填写宠物编号");
        }
        // 判断是否有填写宠物名称。
        if (StringUtil.isNullOrEmpty(post.get("names"))) {
            return R.error("请填写宠物名称");
        }
        // 判断是否有填写宠物类别。
        if (StringUtil.isNullOrEmpty(post.get("category"))) {
            return R.error("请填写宠物类别");
        }
        // 判断是否有填写年龄。
        if (StringUtil.isNullOrEmpty(post.get("age"))) {
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Pet entityData, Map post) {
        // 判断是否有填写宠物编号。
        if (StringUtil.isNullOrEmpty(post.get("numbers"))) {
            return R.error("请填写宠物编号");
        }
