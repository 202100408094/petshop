...
public class ClassificationsService {
    ...
    // 获取数据库操作类mapper
    @Resource
    private ClassificationsMapper mapper;

    /**
     *   根据Names字段参数获取一行数据
     */
    public Classifications findByNames(String username) {
        // 新建商品分类模块实体类Classifications
        Classifications pojo = new Classifications();
        // 设置参数
        pojo.setNames(username);
        // 根据实体类新建QueryWrapper查询条件类
        QueryWrapper<Classifications> queryWrapper = Wrappers.query(pojo);
        Classifications row = mapper.selectOne(queryWrapper);
        return row;
    }
     ...
     */
    public Classifications findByNames(String username, Integer uid) {
        // 新建商品分类模块实体类Classifications
        Classifications pojo = new Classifications();
        // 设置参数
        pojo.setNames(username);
     ...
     *  根据id 获取一行数据
     */
    public R<Classifications> findById(Integer id) {
     ...
     *  根据Wrapper 对象进行数据筛选
     */
    public R<List<Classifications>> selectAll(Wrapper<Classifications> query) {
    ...
    public R<List<Classifications>> selectAll() {
        QueryWrapper<Classifications> wrapper = Wrappers.query();
    ...
    public R selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Classifications> wrapper = mapToWrapper(map);
        // 设置排序
     ...
     */
    public QueryWrapper<Classifications> mapToWrapper(Map<String, Object> map) {
        // 创建 QueryWrapper 对象
        QueryWrapper<Classifications> wrapper = Wrappers.query();
    ...
    public R selectPages(QueryWrapper<Classifications> wrapper, IPage page) {
     ...
     * @return 是否处理成功
     */
    public R insert(Classifications entityData, Map post) {
        // 判断是否有填写分类名称。
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Classifications entityData, Map post) {
        // 判断是否有填写分类名称。
        if (StringUtil.isNullOrEmpty(post.get("names"))) {
            return R.error("请填写分类名称");
        }
