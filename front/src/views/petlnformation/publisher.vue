<template>
    <div class="views-petlnformation-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 宠物资讯查询 </span>
                    </div>
                </template>
                ...
                <div class="form-search">
                    <el-form @submit.prevent.stop :inline="true" size="small">
                        ...
                        <el-form-item label="分类">
                            <el-select v-model="search.classification"
                                ><el-option label="请选择" value=""></el-option><e-select-option type="option" module="classification" value="id" label="names"></e-select-option
                            ></el-select>
                        </el-form-item>
                        <el-form-item>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="classification" label="分类" width="120">
                        <template #default="{row}"> <e-select-view module="classification" :value="row.classification" select="id" show="names"></e-select-view> </template>
                    </el-table-column>
                    <el-table-column prop="rate" label="点击率" width="120">
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/petlnformation/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/petlnformation/updt?id='+row.id)">编辑</el-button>
                                </el-tooltip>
...
<script setup>
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canPetlnformationDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/api/petlnformation/selectPublisher", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
