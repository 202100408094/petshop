<template>
    <div class="views-classification-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 资讯分类查询 </span>
                    </div>
                </template>
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/classification/updt?id='+row.id)">编辑</el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canClassificationSelect, useClassificationSelect, canClassificationDelete } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canClassificationDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/api/classification/selectPages", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
